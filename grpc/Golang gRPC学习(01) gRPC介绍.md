...
![image.png](../images/gepcserver-grpc-request-response-grpcstub.png)
...
## 为什么还是有很多公司用RESTful
以函数为基础的模式（RPC），如果服务端添加了一个函数，那么客户端也必须添加一个函数。<br />而以资源为操作基础的RESTful，URI对应的服务变化了，客户端一般是不需要关心和更新的，它对客户端是透明的。<br />所以RPC的操作变更，有时比RESTful的复杂，因为RCP需要2边都要添加代码，也就增加了工作量。

并且json格式是可阅读的格式，gRPC编码后是二进制格式，对人不友好，不利于阅读.<br />调试排错RPC也比RESTful难，毕竟HTTP大家都很熟悉，而RPC协议一般是自定义协议，需要程序员自己去研究。
